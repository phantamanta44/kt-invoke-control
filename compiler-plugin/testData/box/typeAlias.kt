...
fun <T> runOp(f: Op<T>, arg: T): T {
    return (@ICUnchecked("enclave") f(arg))
}
...
fun box(): String {
    ...
    println(runOp(test9, 42))
    println(runOp(test10, "foot"))
    test11 = { it - 1 }
    println(runOp(test11, 42))
    test12 = { it.uppercase() }
    return runOp(test12, "ok")
}
